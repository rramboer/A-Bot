            ...
            if((player[0] + player[1]) > (opp[0] + opp[1])) {
                if(player[0] == player[1]) {
                    setTimeout(((db, client, channel, user, betAmount) => {
                        ...
                        client.channels.cache.get(channel.id).send("Nice 🤯! You win double your bet, gaining " + (betAmount * 2) + " coins!");
                    ...
                    setTimeout(((db, client, channel, user, betAmount) => {
                        ...
                        client.channels.cache.get(channel.id).send("Nice 🤯! You win back your bet, gaining " + (betAmount) + " coins!");
                    }), 6000, db, client, channel, user, betAmount);
                }
            } else if((player[0] + player[1]) < (opp[0] + opp[1])) {
                setTimeout(((db, client, channel, user, betAmount) => {
